#!/usr/bin/env bash
# =============================
# 🔍 Validates that .devcontainer/variables/.env exists and has required variables
...
ENV_FILE="${DEVCONTAINER_DIR}/variables/.env"
EXAMPLE_FILE="${DEVCONTAINER_DIR}/variables/example.env"
...
if [ ! -f "$ENV_FILE" ]; then
    echo ""
    echo -e "${RED}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${RED}║  ❌  ERROR: .devcontainer/variables/.env file not found!    ║${NC}"
    echo -e "${RED}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${YELLOW}The devcontainer requires a ${BOLD}.devcontainer/variables/.env${NC}${YELLOW} file to run.${NC}"
    echo ""
    echo -e "To create one, copy the example and fill in your API keys:"
    echo ""
    echo -e "  ${GREEN}cp .devcontainer/variables/example.env .devcontainer/variables/.env${NC}"
    echo ""
    echo -e "Then edit ${BOLD}.devcontainer/variables/.env${NC} and add your keys."
    echo -e "See ${BOLD}.devcontainer/variables/example.env${NC} for all available variables."
    echo ""
    exit 1
fi
...
if [ "$ACTIVE_LINES" -eq 0 ]; then
    echo ""
    echo -e "${RED}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${RED}║  ⚠️   WARNING: .devcontainer/variables/.env is empty!       ║${NC}"
    echo -e "${RED}╚══════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo ""
    echo -e "Reference: ${BOLD}.devcontainer/variables/example.env${NC}"
    echo ""
    exit 1
fi
...
if [ -z "$OPENAI_KEY" ]; then
    echo ""
    echo -e "${YELLOW}⚠️  WARNING: OPENAI_API_KEY is not set in .devcontainer/variables/.env${NC}"
...
# -------------------------------------------------------
# 5. Detect whether the host can expose NVIDIA GPUs
# -------------------------------------------------------
